...
def options_trader_instruction(symbol: str | None) -> str:
    ...
    return (
        ...
        "When an option structure is proposed, include a structured option strategy object with legs, expiry, strike, side, quantity, debit/credit, max loss, max profit, breakeven, Greeks snapshot, liquidity filter, contract multiplier, and cash risk fields. "
        "Tie every structure to the vol view, directional view, expiry, strike area, liquidity, Greeks, cash premium/max loss, risk budget, and no-trade conditions. "
