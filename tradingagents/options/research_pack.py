...
def _render_markdown(pack: dict[str, Any]) -> str:
    ...
    lines = [
        f"# 期权研究包 — {pack['product']} {pack['trade_date']}",
        "",
        "## 执行摘要",
        f"- 选择模式: {pack['selection_mode']}",
        f"- 入选策略: {pack['selected_strategy']}",
        f"- 入选决策等级: {summary.get('selected_decision')}",
        f"- 入选评分: {summary.get('selected_score')}",
        f"- 风险预算现金: {summary.get('risk_budget_cash')}",
        f"- 风险预算使用率: {summary.get('selected_risk_budget_utilization')}",
        f"- 风险预算状态: {summary.get('risk_budget_status')}",
        f"- 最差情景 PnL 现金: {summary.get('worst_scenario_pnl_cash')}",
        f"- 回放最大回撤现金: {summary.get('replay_max_drawdown_cash')}",
        f"- 回放胜率: {summary.get('replay_win_rate')}",
        "",
        "## 策略筛选",
        ...
        "",
        "## 研究包假设",
        "- 全流程无副作用：不会自动下单，飞书交付仅生成待发送 payload。",
        "- 价格口径：除非明确要求结算/风控口径，否则使用期权 close + 期货 close。",
        "- 本研究包是可审计的投研工作流，不是自动执行指令。",
    ]
