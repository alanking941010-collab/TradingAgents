...
def test_options_symbol_enables_options_tools_and_volatility_first_prompt_for_market_analyst():
    ...
    assert "get_option_strategy_replay" not in llm.bound_tool_names
    assert "get_option_research_pack_hermes_cron_spec" not in llm.bound_tool_names
...
def test_trading_graph_tool_nodes_include_options_tools_for_analyst_tool_execution():
    ...
    market_tools = getattr(tool_nodes["market"], "tools_by_name", {})
    assert "get_option_strategy_selection" in market_tools
    assert "get_option_research_pack" in market_tools
    assert "get_option_analytics_report" not in market_tools
    assert "get_option_strategy_replay" not in market_tools
    assert "get_option_research_pack_hermes_cron_spec" not in market_tools
