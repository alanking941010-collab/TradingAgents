...
Phase 22B adds an optional TradingAgents debate attachment layer:

```text
tradingagents/options/agent_debate.py
```
...
Phase 22E hardens the timeout boundary around `--with-agent-debate`. Root cause of the previous full-graph timeout: the live debate path called synchronous `TradingAgentsGraph.propagate()` / `graph.invoke()` inside `agent_debate_provider(pack)`, and the daily workflow wrote per-symbol artifacts only after that provider returned. If a live graph node/tool/LLM call stalled or the multi-node graph exceeded the outer shell timeout, no partial debate and no deterministic research-pack artifacts were written. `graph-live-safe` is now the default mode for `--with-agent-debate`; it runs the live graph in an isolated worker process, terminates the worker on the configured wall-clock timeout, returns a reportable `agent_debate.status="timeout"` payload, and still writes JSON/Markdown/DOCX artifacts. Use `--agent-debate-mode graph-live` only when you explicitly want the old fail/timeout behavior.

```bash
--agent-debate-mode graph-live-safe \
--agent-debate-timeout-seconds 300
```
...
The checkpoint captures completed debate/report sections and per-event diagnostics such as elapsed time, delta time, message type, tool-call names, message content length, and completed sections. If timeout occurs, the timeout debate payload loads the latest checkpoint and appends any completed sections to the final Markdown/DOCX timeout explanation. A 90-second Kimi diagnostic on CU showed the graph reached seven streamed events before timeout; the biggest observed tool messages were `get_option_strategy_selection` (~53k chars) and `get_option_strategy_report` (~14k chars), after which the next LLM synthesis did not return before timeout.

Phase 22G reduces that live-graph latency pressure by making LLM-facing options tools compact by default. `get_option_strategy_selection`, `get_option_strategy_report`, `get_option_strategy_scenarios`, and `get_option_research_pack` now expose `detail_level="compact|full"`; compact mode removes nested full strategy/candidate/delivery payloads, trims per-leg scenario values, and uses a smaller default scenario grid, while `detail_level="full"` remains available for audit/debug. The live market analyst and graph tool node now expose only the compact default options menu: `get_option_trade_context`, `get_option_strategy_selection`, and `get_option_research_pack`; heavy audit/report/scenario tools stay callable directly but are not bound to the live analyst graph by default.

Phase 22H addresses the post-22G case where checkpoint diagnostics show all required live sections have already completed (`Market Analyst`, `Bull/Bear Research Debate`, `Research Manager`, `Trader`, `Risk Debate`, `Portfolio Manager`) but the outer worker still times out during graph finalization/return. On timeout/failure, `load_agent_debate_checkpoint()` now marks whether the latest checkpoint is complete enough for reporting. If it is complete and has a final decision (or Portfolio Manager content), the provider promotes the result to `agent_debate.status="checkpoint_complete"` with `source="tradingagents_graph_live_checkpoint_complete"` and uses the checkpointed sections/final decision in Markdown/DOCX instead of reporting a pure `timeout`. Incomplete checkpoints still remain `timeout` / `failed` and list `missing_required_sections`.

Phase 22B also adds selector constraint handling:

```bash
--constraint-mode strict   # default for core APIs; old behavior
--constraint-mode relaxed  # CLI default; softens liquidity/risk-budget/credit-filter failures
```

In `relaxed` mode, failed liquidity/risk-budget/credit-quality checks remain visible as warnings and score penalties, but do not automatically force `no_trade`. This is intended for trader review lists, not automatic execution.

Phase 21 adds a daily/batch workflow entrypoint:

```text
tradingagents/options/research_pack_workflow.py
...
```bash
python scripts/build_options_research_pack_daily.py \
  --symbol CU \
  --symbol AU \
  --date 2026-05-01 \
  --directional-bias neutral \
...
`build_daily_options_research_pack_hermes_cron_spec(...)` and CLI `--stdout hermes-cron-spec` describe the no-agent cron command for daily/after-close delivery, but they do not create a Hermes cron job. Confirm schedule, target, and symbol list before creating a live recurring job.
...
- `market_analyst` keeps `get_stock_data` and `get_indicators`, and for supported SHFE metals option symbols now binds only the compact default options menu: `get_option_trade_context`, `get_option_strategy_selection`, and `get_option_research_pack`. Heavy audit/report/scenario tools remain available as direct utilities but are not exposed to the live market analyst by default after Phase 22G.
...
- Phase 22F live graph checkpoint diagnostics adds streaming partial checkpoints under `agent_debate_checkpoints/`, loads latest partial sections into timeout/failure debate payloads, and records per-event timing/tool/message-size diagnostics. Live Kimi diagnostic showed timeout pressure from large option tool outputs (`get_option_strategy_selection` ~53k chars, `get_option_strategy_report` ~14k chars) followed by slow LLM synthesis.
- Phase 22G compact LLM-facing options tools makes `get_option_strategy_selection`, `get_option_strategy_report`, `get_option_strategy_scenarios`, and `get_option_research_pack` default to `detail_level="compact"` with `detail_level="full"` opt-in for audit/debug. The live market analyst/graph now binds only `get_option_trade_context`, `get_option_strategy_selection`, and `get_option_research_pack` to reduce full-graph timeout pressure.
